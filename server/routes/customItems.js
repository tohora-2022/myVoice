...
router.get('/', checkJwt, async (req, res) => {
  const userId = dbUsers.findUserId(req.user?.sub)
  db.getCustomItems(userId)
...
router.post('/upload', checkJwt, async (req, res) => {
  ...
  let customItem = { }
  if (req.files === null) {
    console.log('req.files === null', req.files === null)
    return res.status(400).json({ msg: 'No file uploaded' })
  }

  const file = req.files.file

  db.addCustomItem(word, 'tempPath', userId)
    .then(createdItem => {
      customItem = createdItem
      console.log('customItemId', customItem)
      ...
      return db.updateCustomItemPath(customItem.id, `${__dirname}/../public/images/custom/${customItem.id}`)
    })
    .then(updatedItem => {
